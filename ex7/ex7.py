...
# If using the above code may cause the following error:
# IndexError: arrays used as indices must be of integer (or boolean) type
# Thus I changed line 140 into the following codes(lines 144-146)
X_recovered = np.zeros(X.shape)
for i in range(len(centroids)):
	X_recovered[np.where(idx==i)] = centroids[i]
